...
const stats = [
  { value: "80+", label: "Projects successful", desc: "Proven track record of achieving business goals." },
  { value: "50+", label: "Clients satisfied", desc: "Our clients trust us to deliver excellence for their projects." },
  { value: "5+", label: "Awards Won", desc: "Decades of expertise powering your success." },
  { value: "3", label: "Countries Covered", desc: "Broad experience across diverse markets." },
];
          ...
          <div className="flex flex-col justify-start">
            <h2 className="font-[family-name:var(--font-display)] text-[2.5rem] leading-[1.2] font-medium text-[#1A1A1A] mb-6">
              ...
              <div className="space-y-6 text-base text-[#1A1A1A] leading-relaxed">
                <p>
                  SR Glanz is your partner in IT success. We provide specialized staffing solutions and comprehensive implementation support to help enterprises build, scale, and sustain cutting-edge technology systems.
                </p>
              <p>
              ...
              <div className="mt-8">
                <Link 
                  href="/about-us" 
                  className="inline-flex items-center justify-center px-8 py-3 bg-[#febd73] hover:bg-[#fff1de] text-[#1A1A1A] text-sm font-semibold rounded-full transition-all duration-300 transform active:scale-95"
                >
                  Meet SR Glanz
                </Link>
              </div>
          ...
          {stats.map((stat, idx) => (
            <div key={idx} className="flex flex-col items-start">
              <div className="font-[family-name:var(--font-display)] text-[3.5rem] lg:text-[4rem] font-semibold text-[#1A1A1A] leading-none mb-3 flex items-start">
                {stat.value.includes('+') ? (
                  <>{stat.value.replace('+', '')}<span className="text-[2.5rem] ml-0.5 mt-1 align-top">+</span></>
                ) : stat.value}
              </div>
              <h3 className="font-[family-name:var(--font-display)] text-xl font-semibold text-[#1A1A1A] mb-2">
                {stat.label}
              </h3>
              <p className="text-[#595959] text-sm leading-relaxed">{stat.desc}</p>
            </div>
          ))}
        </div>
      </div>
